...
class MultiProcess_Scheduler_By_Redis extends MultiProcess_Scheduler_Base {
    ...
    private function hSet($table, $field, $value) {
        $res = (self::$objRedis instanceof Redis) ? self::$objRedis->hSet($table, $field, $value) : false;
        if (false === $res) {
            self::renewRedisConnect();
    ...
    private function hSetNx($table, $field, $value) {
        $res = (self::$objRedis instanceof Redis) ? self::$objRedis->hSetNx($table, $field, $value) : false;
        if (false === $res) {
            self::renewRedisConnect();
    ...
    private function hDel($table, $field) {
        ...
        if(false === $res) {
            ...
            if(false === $res) {
                trigger_error('Redis delete an hash field Failed!', E_USER_WARNING);
                throw new Exception('Redis删除一个哈希表字段值失败', self::ERROR_REDIS_WRONG);
            }
        }
        return true;
    }
    ...
    private static function renewRedisConnect() {
        ...
        while ((false === $isConnect || '+PONG' != self::$objRedis->ping()) && --$retryCount) {
            $waitTime = rand(3000 * $retryCount,1000000);
            usleep($waitTime);
            $isConnect = self::$objRedis->connect(self::REDIS_SERVER, self::REDIS_PORT);
		}
        if ($retryCount == 0) {
            trigger_error('Redis can not be used normally!', E_USER_WARNING);
            throw new Exception('Redis 无法正常使用!', MultiProcess_Base::ERROR_REDIS_WRONG);
        }
    }
}